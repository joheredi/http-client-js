...
describe("RestorePollerFile", () => {
   ...
   */
  it("should render RestorePollerOptions interface", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
    ...
    const result = renderWithContext(program, client, [method]);
   ...
   */
  it("should render restorePoller function with correct signature", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should render restorePoller function body with correct logic", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should generate deserializeMap with LRO operation entries", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should handle multiple LRO operations in deserialize map", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should render getDeserializationHelper function", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should render utility functions", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should render DeserializationHelper interface (not exported)", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should collect LRO operations from child clients", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
   ...
   */
  it("should make RestorePollerOptions referenceable via refkey", async () => {
    const runner = await TesterWithService.createInstance();
    const { program } = await runner.compile(t.code`@get op test(): void;`);
