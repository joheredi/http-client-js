...
describe("Versioning.Added", () => {
  ...
  /**
   * Tests v2InInterface operation added in API version v2 via InterfaceV2.
   * With hierarchy-client: false (default), the operation is flattened onto the root client.
   */
  it("should call v2InInterface operation", async () => {
    ...
    const result = await client.v2InInterface(body);
    expect(result.prop).toBe("foo");
    expect(result.enumProp).toBe("enumMember");
    expect(result.unionProp).toBe("bar");
  });
