...
export function SendOperation(props: SendOperationProps) {
  ...
  // Skip the auto-generated accept header when a user-defined Accept header
  // exists in headerParams — the user-defined one will carry its own accessor
  // expression (with clientDefaultValue fallback if applicable).
  const hasUserDefinedAcceptHeader = headerParams.some((p) =>
    isAcceptHeader(p),
  );
  const acceptHeader = hasUserDefinedAcceptHeader
    ? undefined
    : getAcceptHeader(operation);
...
function getHeaderParameters(
  ...
  return operation.parameters.filter(
    (p): p is SdkHeaderParameter =>
      ...
      !isAutoGeneratedAcceptHeader(p),
  );
}

/**
 * Checks whether an HTTP header parameter is an auto-generated Accept header.
 *
 * User-defined Accept headers (e.g., `@header("Accept") accept?: string` with
 * `@clientDefaultValue`) should be treated as custom headers so their default
 * values are applied. Only auto-generated Accept headers (synthetic params
 * created by TCGC) are filtered out — their value is computed from response
 * content types via `getAcceptHeader()`.
 *
 * @param param - The HTTP header parameter to check.
 * @returns `true` if this is an auto-generated Accept header to be filtered out.
 */
function isAutoGeneratedAcceptHeader(param: SdkHeaderParameter): boolean {
  if (!isAcceptHeader(param)) return false;
  const corresponding = param.correspondingMethodParams[0];
  if (!corresponding || corresponding.kind !== "method") return true;
  return isAutoGeneratedHeader(corresponding);
}
...
function buildHeaderEntries(
  ...
  for (const header of headerParams) {
    ...
    if (conditions.length > 0) {
      ...
      entries.push(
        code`...(${condition} ? { "${headerKey}": ${wrappedAccessor} } : {})`,
      );
    } else {
      entries.push(code`"${headerKey}": ${wrappedAccessor}`);
    }
  }
