...
export function typeHasDeserializerDeclaration(type: SdkType): boolean {
  switch (type.kind) {
    case "model":
      ...
      return (type.usage & UsageFlags.Output) !== 0 ||
...
export function typeHasSerializerDeclaration(type: SdkType): boolean {
  switch (type.kind) {
    ...
    case "union":
      return !!(type.name && !type.isGeneratedName &&
...
```tsx
<InterfaceMember name={property.name} type={propertyType} optional={property.optional} />
```
...
Instead of normalizing names manually, use Alloy's `resolveSymbolName()` or `useNamePolicy()` hooks to ask the framework what name a property *would* get.
...
{(prop) => {
  ...
  const valueExpr = getXmlObjectSerializationExpression(prop.type, accessor, prop);
...
{(prop) => {
  ...
  const accessor = `item["${prop.name}"]`;
  let valueExpr = getXmlObjectSerializationExpression(prop.type, accessor, prop);
...
function xmlNeedsTransformation(type: SdkType): boolean {
  switch (type.kind) {
    ...
    case "utcDateTime":
    case "plainDate": return true;
    case "bytes": return true;
    case "nullable": return xmlNeedsTransformation(type.type);
...
import type { SdkModelPropertyType, SdkType } from "@azure-tools/typespec-client-generator-core";
